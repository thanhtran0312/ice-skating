...
reminder: after each step, run a sanity check to see if len(start_events/end_events) == n_trials because trigger samples are not very stable (otherwise you realize in the end and rerun all steps so many times, talking from experience); e.g:

- events = mne.find_events(signal,stim_channel="STI101",shortest_event=1,initial_event=True)
