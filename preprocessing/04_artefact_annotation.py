...
interactive = bool(config["annotation"].get("interactive", True))
...
detrend_dir = deriv_root / bids_subject / bids_session / "meg"
...
detrend_files = [
    detrend_dir / f"{bids_prefix}_run-{run}_desc-detrend_meg.fif"
...
annotation_files = {
    detrend_file: annotation_dir / detrend_file.name.replace(
        "_desc-detrend_meg.fif", "_desc-annotated_meg.fif"
    )
    for detrend_file in detrend_files
}
annotation_csv_files = {
    file: annotation_dir / file.name.replace(
        "_desc-detrend_meg.fif", "_desc-annotations.csv"
    )
    for file in detrend_files
}
manual_eye_annotation_files = {
    file: manual_eye_annotation_dir / file.name.replace(
        "_desc-detrend_meg.fif", "_desc-manualeye-annot.fif"
    )
    for file in detrend_files
}
...
if manual_annotation_run_index >= len(detrend_files):
    raise IndexError(
        "annotation.manual_annotation_run_index is outside the configured run list."
    )
manual_file = detrend_files[manual_annotation_run_index]
...
#     )
# else:
#     print("Skipping interactive manual annotation (interactive=false); "
#           "using previously saved manual eye annotations.")
...
run_summaries = []
for file in detrend_files[1:]:
    print(f"\nAnnotating {file.name}")
    run_summaries.append(annotate_one_run(file))
