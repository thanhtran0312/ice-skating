...
% Diagnostic heartbeat trigger every 5 s of UNIQUE movie content.
heartbeatIntervalSec = 5;
heartbeatTrigger = 200;   % keep separate from all experimental trigger codes
audiovolume = 0;
...
refrate = 1/Screen('GetFlipInterval', win);
msg = 'Change the display to 100Hz';
if refrate > 110
    sca
    error(msg)
end
...
for itrial = 1:size(condmat,2)
    replaying = false;
    replayOccIdx = NaN;
    replayEndMovieTime = NaN;
    replayContinuationTrigger = NaN;


    trig_trial_start = vid2disp(itrial);
    trig_trial_end   = 40 + currentcondition(itrial);
    ...
    % Monotonic 5-s checkpoint index for the heartbeat trigger; reset per trial.
    lastHeartbeatIdx = 0;
    
    occ_count_within_trial = 1;
    [movie, movieduration, fpsn, imgw, imgh, ~, ~] = Screen('OpenMovie', win, movienames);
    nMovieFrames = round(movieduration*fps);
    WaitSecs(2)
    ...
    vblTimes = [];
    missedTimes = [];
    occMovieTimes = nan(1, n_occ_this_trial);
    occVBLs = nan(1, n_occ_this_trial);
    requestedRestartTimes = nan(1, n_occ_this_trial);
    restartMovieTimes = nan(1, n_occ_this_trial);
    restartVBLs = nan(1, n_occ_this_trial);
    continuationMovieTimes = nan(1, n_occ_this_trial);
    continuationVBLs = nan(1, n_occ_this_trial);
    ...
    while 1
        ...
        if abs(movierate) > 0
            [tex, movieFrameTime] = Screen('GetMovieImage', win, movie, blocking);
            ...
            isContinuationFrame = replaying && movieFrameTime > replayEndMovieTime;
            isLastFrame = framecount >= round(movieduration*fps);
            isFirstFrame = (framecount==1);
            Screen('FillRect', win, [black black black], photodiodepos);

            if isContinuationFrame
            
                continuationMovieTimes(replayOccIdx) = movieFrameTime;
            
                if MEG
                    triggerPulse = [1 0] .* replayContinuationTrigger;
            ...
            elseif isFirstFrame
                ...
                if MEG
                    ...
                    Datapixx('SetDoutSchedule', 1.0/refrate, 1000, 2);
                    Datapixx('StartDoutSchedule');
                    % commit immediately before the flip
                    Datapixx('RegWrVideoSync');
                end
            else
                Screen('FillRect', win, [black black black], photodiodepos);
                ...
                %% Diagnostic heartbeat trigger every 5 s of UNIQUE movie content
                %
                % Armed before this frame's Flip so RegWrVideoSync ties the
                % heartbeat to this movie-frame presentation. Uses a
                % monotonic checkpoint index (based on movieFrameTime) so a
                % checkpoint cannot fire again if this point in the movie is
                % revisited during the 1-s occlusion replay.
                hbIdx = floor(movieFrameTime / heartbeatIntervalSec);
                if hbIdx > lastHeartbeatIdx && hbIdx >= 1 && movieFrameTime < (movieduration - heartbeatIntervalSec)
                    if MEG
                        triggerPulse = [1 0] .* heartbeatTrigger;
                        Datapixx('StopDoutSchedule');
                        Datapixx('WriteDoutBuffer', triggerPulse);
                        Datapixx('SetDoutSchedule', 1.0/refrate, 1000, 2);
                        Datapixx('StartDoutSchedule');
                    ...
                    lastHeartbeatIdx = hbIdx;
                end
            end
            [vbl, stimOnset, flipTimestamp, missed] = Screen('Flip', win);    
            lastMovieVBL = vbl;
            Screen('Close', tex);
            if isContinuationFrame
                continuationVBLs(replayOccIdx) = vbl;
                ...
                output(triggercount).trigger   = replayContinuationTrigger;
                output(triggercount).task_num  = replayOccIdx;
                output(triggercount).task_onset = GetSecs;
            
                if trackeye
                    Eyelink('Message', sprintf( ...
                        'TRIGGER %d movie_continue occ%d', ...
                        replayContinuationTrigger, replayOccIdx));
                end
                                    
                fprintf(['CONTINUATION occ %d | movie %.3f | ' ...
                         'restart %.3f | physical replay %.3f ms\n'], ...
                    replayOccIdx, ...
                    movieFrameTime, ...
                    restartMovieTimes(replayOccIdx), ...
                    1000 * (vbl - restartVBLs(replayOccIdx)));
            ...
            if isLastFrame
                ...
                trialEndVBL = Screen('Flip', win, lastMovieVBL + 1.5/refrate);
                if trackeye
            ...
            movieTimes(framecount) = movieFrameTime;
            vblTimes(framecount) = vbl;
            missedTimes(framecount) = missed; 
            ...
            if trackeye
                ...
                if ~isempty(sample) && isstruct(sample)
                    ...
                    if gazeOutsideCounter == fixBreakToleranceFrames && ~currentlyOutside
                        ...
                        movieFrameIdx = round(movieFrameTime * fps) + 1;
                        Eyelink('Message', sprintf('FIXATION_BREAK trial%d break%d frame%d', ...
                            itrial, fixBreakCount, movieFrameIdx));
                    end
                end
            end
        ...
        %% task: occlusion
        occ_frame = occ_onset(occ_count_within_trial);
        occTime = occ_frame / fps;
        if movieFrameTime >= occTime 
             trig_occ_onset    = 50 + (occ_count_within_trial-1)*6 + currentcondition(itrial);
             trig_task_onset   = 70 + (occ_count_within_trial-1)*6 + currentcondition(itrial);
             trig_movie_restart = 110 + (occ_count_within_trial-1)*6 + currentcondition(itrial);
             trig_movie_continue = 130 + (occ_count_within_trial-1)*6 + currentcondition(itrial);
             occlusion_start = movieFrameTime;
             occMovieTimes(1,occ_count_within_trial) = occlusion_start;
             movierate = 0;
             Screen('PlayMovie', movie, movierate, 0, audiovolume);
             ...
             occVBL = Screen('Flip', win);    
             occVBLs(1,occ_count_within_trial) = occVBL;
             if trackeye
             ...
             if any(currentcondition(itrial) == [1 2 3])
                ...
                if correct_response_for_this_trial(occ_count_within_trial) == 1
                    movieFrameIdx = round(movieFrameTime * fps) + 1;
                    targetTime = movieFrameTime + occ_time;
             ...
             elseif any(currentcondition(itrial) == [4 5 6])
                ...
                probeFrame = round((movieFrameTime + occ_time) * fps) + 1;
                if correct_response_for_this_trial(occ_count_within_trial) == 1
                    skeleton_x = keypoints_new(vid2disp(itrial),:,1,probeFrame);
                    skeleton_y = keypoints_new(vid2disp(itrial),:,2,probeFrame);

                elseif correct_response_for_this_trial(occ_count_within_trial) == 0
                    skeleton_x = keypoints_new(vid2disp(itrial),:,1, probeFrame);
                    skeleton_y = keypoints_new(vid2disp(itrial),:,2, probeFrame);
                
                    pelvis_x_now  = (keypoints_new(vid2disp(itrial),12,1,probeFrame) + ...
                                     keypoints_new(vid2disp(itrial),13,1,probeFrame)) / 2;
                    pelvis_y_now  = (keypoints_new(vid2disp(itrial),12,2,probeFrame) + ...
                                     keypoints_new(vid2disp(itrial),13,2,probeFrame)) / 2;
                
                    pelvis_x_100  = (keypoints_new(vid2disp(itrial),12,1,probeFrame + 200) + ...
                                     keypoints_new(vid2disp(itrial),13,1,probeFrame + 200)) / 2;
                    pelvis_y_100  = (keypoints_new(vid2disp(itrial),12,2,probeFrame + 200) + ...
                                     keypoints_new(vid2disp(itrial),13,2,probeFrame + 200)) / 2;
            ...
            if ~isnan(given_response_for_this_trial)
                resp_trig = 100 + (occ_count_within_trial-1)*2 + given_response_for_this_trial; 
            else
                resp_trig = 105 + occ_count_within_trial;
            end            
            if MEG 
            ...
            newtime = max(0, occlusion_start - 1);
            requestedRestartTimes(1,occ_count_within_trial) = newtime;

            Screen('SetMovieTimeIndex', movie, newtime);
            movierate = 1;
            Screen('PlayMovie', movie, movierate, 0, audiovolume);
            
            % Get the first actual resumed movie frame
            [tex, restartMovieTime] = Screen('GetMovieImage', win, movie, 1);
            restartMovieTimes(1, occ_count_within_trial) = restartMovieTime;
            Screen('DrawTexture', win, tex, [], movierect);
            Screen('DrawLine', win, fix_color, ...
                xCenter-crosssize, yCenter, xCenter+crosssize, yCenter, crosswidth);
            Screen('DrawLine', win, fix_color, ...
                xCenter, yCenter-crosssize, xCenter, yCenter+crosssize, crosswidth);
            Screen('FillOval', win, black, ...
                [xCenter-dotsize yCenter-dotsize xCenter+dotsize yCenter+dotsize], ...
                dotsize*2);
            ...
            restartVBL = Screen('Flip', win);
            restartVBLs(1, occ_count_within_trial) = restartVBL;

            replaying=true;

            replayOccIdx = occ_count_within_trial;
            replayContinuationTrigger = trig_movie_continue;

            replayEndMovieTime = occlusion_start;
            Screen('Close', tex);
            
            if trackeye
                Eyelink('Message', sprintf('TRIGGER %d movie_restart occ%d', trig_movie_restart, occ_count_within_trial));
            end
            output(triggercount).trial     = itrial;
            output(triggercount).video     = vid2disp(itrial);
            output(triggercount).condition = currentcondition(itrial);
    ...
    fprintf('Frames returned: %d\n', length(movieTimes));
    
    fprintf('First movie time: %.6f s\n', movieTimes(1));
    fprintf('Last movie time:  %.6f s\n', movieTimes(end));
    
    fprintf('Movie timestamp span: %.6f s\n', ...
        movieTimes(end) - movieTimes(1));
    
    fprintf('VBL span: %.6f s\n', ...
        vblTimes(end) - vblTimes(1));
    
    fprintf('Mean movie dt: %.9f s\n', ...
        mean(diff(movieTimes)));
    
    fprintf('Mean VBL dt: %.9f s\n', ...
        mean(diff(vblTimes)));
    
    fprintf('Missed flips: %d\n', sum(missedTimes > 0));
    fprintf('Measured refresh rate: %.6f Hz\n', refrate);
    fprintf('Measured IFI: %.9f s\n', 1/refrate);
    fprintf('Expected movie frames: %d\n', round(movieduration * fps));
    fprintf('Returned movie frames: %d\n', framecount);
    for iOcc = 1:n_occ_this_trial
        fprintf(['occ %d | movie %.3f | requested %.3f | restart movie %.3f | ' ...
                 'source replay %.3f | VBL replay %.3f ms\n'], ...
            iOcc, ...
            occMovieTimes(iOcc), ...
            requestedRestartTimes(iOcc), ...
            restartMovieTimes(iOcc), ...
            occMovieTimes(iOcc) - restartMovieTimes(iOcc), ...
            1000 * (occVBLs(iOcc) - restartVBLs(iOcc)));
    end
    fprintf("last movie frame - end flip = %.3f ms", 1000*(trialEndVBL - lastMovieVBL))
    ...
    timing(itrial).video = vid2disp(itrial);
    timing(itrial).condition = currentcondition(itrial);
    
    timing(itrial).occMovieTimes = occMovieTimes;
    timing(itrial).occVBLs = occVBLs;
    
    timing(itrial).requestedRestartTimes = requestedRestartTimes;
    timing(itrial).restartMovieTimes = restartMovieTimes;
    timing(itrial).restartVBLs = restartVBLs;
    
    timing(itrial).sourceReplayDuration = ...
        occMovieTimes - restartMovieTimes;
    
    timing(itrial).vblReplayDuration = ...
        continuationVBLs - restartVBLs;
    timing(itrial).movieTimes = movieTimes;
    timing(itrial).vblTimes = vblTimes;
    timing(itrial).missedTimes = missedTimes;
    
    timing(itrial).firstMovieTime = movieTimes(1);
    timing(itrial).lastMovieTime = movieTimes(end);
    
    timing(itrial).movieTimestampSpan = ...
        movieTimes(end) - movieTimes(1);
    
    timing(itrial).vblSpan = ...
        vblTimes(end) - vblTimes(1);
    
    timing(itrial).refreshRate = refrate;
    timing(itrial).ifi = 1 / refrate;
    
    timing(itrial).lastFrameToEndFlip = ...
        trialEndVBL - lastMovieVBL;
    timing(itrial).continuationMovieTimes = continuationMovieTimes;
    timing(itrial).continuationVBLs = continuationVBLs;
    
    timing(itrial).movieReplayToContinuation = ...
        continuationMovieTimes - restartMovieTimes;
    
    timing(itrial).vblReplayToContinuation = ...
        continuationVBLs - restartVBLs;
end % for loop: trials


%% 7. save
save(fullfile(savedir, filename), 'output', 'fixation_breaks_per_trial','timing');
