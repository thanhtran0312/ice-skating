...
report_file = report_dir / "05_ica.html"
report = mne.Report(title=f"05 ICA - {bids_subject} {bids_session}")
...
# %%
# -----------------------------------------------------------------------------
# Report ICA source time courses (manual stacked plot, all components)
# -----------------------------------------------------------------------------
# ica.plot_sources()'s browser figure caps how many channels are shown per
# screen, independent of `picks`. To guarantee all components render in one
# static figure, extract the source time courses directly and plot them
# manually with a fixed per-component vertical offset.

sources = ica.get_sources(raw_resmpl_all)  # Raw object, one "channel" per IC

rng = np.random.default_rng(config["ica"].get("random_state", 0))
snippet_duration = 10.0
total_duration = raw_resmpl_all.times[-1]
n_snippets = 3
max_start = max(0.0, total_duration - snippet_duration)
starts = sorted(rng.uniform(0, max_start, size=n_snippets))

n_components = ica.n_components_

for start in starts:
    stop = start + snippet_duration
    start_samp = int(start * sources.info["sfreq"])
    stop_samp = int(stop * sources.info["sfreq"])

    data = sources.get_data()[:, start_samp:stop_samp]
    times = sources.times[start_samp:stop_samp]

    offset_step = 5 * np.median(np.std(data, axis=1))
    fig, ax = plt.subplots(figsize=(14, 0.3 * n_components + 2))
    for i in range(n_components):
        ax.plot(times, data[i] + i * offset_step, color="black", linewidth=0.5)

    ax.set_yticks([i * offset_step for i in range(n_components)])
    ax.set_yticklabels([f"ICA{idx:03d}" for idx in range(n_components)])
    ax.set_xlabel("time (s)")
    ax.set_title(f"ICA components (t={start:.1f}-{stop:.1f}s)")
    fig.tight_layout()
    add_figures_to_report(
        fig,
        f"ICA source time courses (t={start:.1f}s)",
        "ICA review",
    )
...
exclude_by_subject = config["ica"]["exclude_components"]
if subject not in exclude_by_subject:
    raise KeyError(
        f"No exclude_components entry for subject '{subject}' in config.toml. "
        f"Add one under [ica.exclude_components] (use [] if not yet reviewed)."
    )

ica.exclude = exclude_by_subject[subject]
...
# %%
# -----------------------------------------------------------------------------
# Report before/after ICA correction (overlaid)
# -----------------------------------------------------------------------------
# raw.plot() can't overlay two raw objects in one figure, so the traces are
# extracted manually and stacked with a fixed vertical offset per channel,
# matching the layout of the MNE browser but with before/after in one panel.

def plot_before_after_overlay(raw_before, raw_after, chs, chan_idxs, duration=5, start=0):
    """Overlay before/after ICA traces for the given channels in one figure.

    Parameters
    ----------
    raw_before, raw_after : mne.io.BaseRaw
        Raw objects to compare (same channels, same sampling).
    chs : list of str
        Channel names to plot (used for y-axis labels).
    chan_idxs : list of int
        Channel indices into raw_before/raw_after matching ``chs``.
    duration : float
    ...
    data_before = raw_before.get_data(picks=chan_idxs)[:, start_samp:stop_samp]
    data_after = raw_after.get_data(picks=chan_idxs)[:, start_samp:stop_samp]
    times = raw_before.times[start_samp:stop_samp]

    # Scale offset relative to signal amplitude so traces don't overlap
    offset_step = 3 * np.nanstd(data_before)

    fig, ax = plt.subplots(figsize=(14, 1.2 * len(chs) + 2))
    for i, ch in enumerate(chs):
        offset = i * offset_step
        ax.plot(times, data_before[i] + offset, color="tab:red", linewidth=0.7,
                label="Before ICA" if i == 0 else None)
        ax.plot(times, data_after[i] + offset, color="tab:blue", linewidth=0.7,
                label="After ICA" if i == 0 else None)

    ax.set_yticks([i * offset_step for i in range(len(chs))])
    ax.set_yticklabels(chs)
    ax.set_xlabel("time (s)")
    ax.set_title("Before vs after ICA correction")
    ax.legend(loc="upper right")
    fig.tight_layout()
    return fig


fig = plot_before_after_overlay(raw_before, raw_after, chs, chan_idxs, duration=5, start=0)
add_figures_to_report(fig, "Before vs after ICA correction", "Before/after")
