...
maxtrignum = 90; % 6 trials * ~15 triggers each = safe upper bound
output = struct(...
...
for itrial = 1:size(condmat,2) % condmat = 30 participants x 6 trials x 6 runs x 2 vid&cond
   % trig should send condition information
    ...
    if any(currentcondition(itrial) == [1,2,3])
        DrawFormattedText(win,'Focus on the posture of the skater and ignore the position.','center',yCenter, black);
        colorID = 2;
    elseif any(currentcondition(itrial) == [4,5,6])
        DrawFormattedText(win,'Focus on the position of the skater and ignore the posture.','center',yCenter, black);
        colorID = 1;
    end
            ...
            Screen('Close', tex);
             if framecount == 1
                if MEG
                    ...
                    Datapixx('RegWr');                           % Commit register writes immediately to hardware.
                end
                if trackeye
...
%     % - **show frame according to condition 123 - posture vs 456 - position
             ...
             elseif any(currentcondition(itrial) == [4 5 6])
                ...
                elseif correct_response_for_this_trial(occ_count_within_trial) == 0 % incongruent
                    % correct posture (shape) from the probe frame
                    skeleton_x = keypoints_new(vid2disp(itrial),:,1, framecount + occ_time*fps);
                    skeleton_y = keypoints_new(vid2disp(itrial),:,2, framecount + occ_time*fps);
                
                    % but shift the whole skeleton to the POSITION from 100 frames later
               ...
               % map keypoints to screen space
                ...
                % white background where movie was
