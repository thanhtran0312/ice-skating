...
# %%
# -----------------------------------------------------------------------------
# Bad-channel decision record
# -----------------------------------------------------------------------------
# Keep the notebook's printed summary as the first text record of which sensors
# were noisy or flat before applying a common bad-channel list.
...
# %%
# -----------------------------------------------------------------------------
# Report first-run raw power spectrum
# -----------------------------------------------------------------------------
# This replaces the notebook's inline ``%matplotlib inline`` PSD display for the
# first raw acquisition run.

first_raw = raws[first_raw_file]
fig = first_raw.compute_psd(fmax=60, n_fft=min(1000, first_raw.n_times)).plot(
    show=False
)
report.add_figure(
    fig,
    title=f"Raw spectra before Maxwell filtering - {first_raw_file.name}",
    section="PSD comparison",
)
plt.close(fig)
...
# %%
# -----------------------------------------------------------------------------
# Report first-run Maxwell-filtered power spectrum
# -----------------------------------------------------------------------------
# Capture the post-SSS PSD so the report contains the before/after comparison
# that was previously inspected inline in the notebook.

first_sss = raws_sss[first_raw_file]
fig = first_sss.compute_psd(fmax=60, n_fft=min(1000, first_sss.n_times)).plot(
    show=False
)
report.add_figure(
    fig,
    title=f"Maxwell-filtered spectra - {first_raw_file.name}",
    section="PSD comparison",
)
plt.close(fig)
...
# %%
# -----------------------------------------------------------------------------
# Save Maxwell-filtered FIF files
# -----------------------------------------------------------------------------
# Raw inputs are not overwritten. Every output receives a BIDS-derivative
# ``desc-maxfilter`` label and is saved under the configured derivatives root.
...
# Save the MNE report
# -----------------------------------------------------------------------------
# The report collects the diagnostic figures that were inline in the notebook
# and stores them next to the other preprocessing derivatives.
